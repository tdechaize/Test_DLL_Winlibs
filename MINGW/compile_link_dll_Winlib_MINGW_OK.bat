...
echo. Lancement du batch de generation d'une DLL et deux tests de celle-ci avec MingW 32 bits ou 64 bits de Winlibs
...
gcc -s -shared -LC:\mingw32\lib -LC:\mingw32\i686-w64-mingw32\lib -Wl,--output-def=dll_core.def -Wl,--out-implib=libdll_core.dll.a -Wl,--dll -o dll_core.dll -m32 -lkernel32 -luser32 dll_core.obj 
type dll_core.def
...
gcc -DBUILD_DLL -DNDEBUG -D_WIN32 -shared -o dll_core.dll -m32 -Wl,--output-def=dll_core.def -Wl,--out-implib,libdll_core.dll.a src\dll_core.c 
type dll_core.def
...
type dll_core_2.def
echo.  ************     Generation et lancement du premier programme de test de la DLL en mode implicite.      *************
gcc -DNDEBUG -D_WIN32 src\testdll_implicit.c -m32 -L. -o testdll_implicit.exe dll_core.dll
REM 	Run test program of DLL with implicit load
testdll_implicit.exe
echo.  ************     Generation et lancement du deuxieme programme de test de la DLL en mode explicite.     ************
gcc -DNDEBUG src\testdll_explicit.c -m32 -o testdll_explicit.exe
REM 	Run test program of DLL with explicit load
testdll_explicit.exe
)
...
REM		-lkernel32 -luser32						-> -lxxxxxxxx set library used by linker to xxxxxxxxx
echo.  ***************          Edition de liens de la DLL avec MingW64 inclus dans Winlibs       *******************
gcc -s -shared -LC:\mingw64\lib -LC:\mingw64\x86_64-w64-mingw32\lib -Wl,--output-def=dll_core64.def -Wl,--out-implib=libdll_core64.dll.a -Wl,--dll -o dll_core64.dll -m64 -lkernel32 -luser32 dll_core64.obj 
type dll_core64.def
...
echo.  ************     Generation et lancement du premier programme de test de la DLL en mode implicite.    *************
gcc -c -DNDEBUG -D_WIN32 -o testdll_implicit64.o -m64 src\testdll_implicit.c
REM 	Options used by linker of CLANG/LLVM compiler
REM 		-s 									-> Strip output file, here dll file.
REM			-m64								-> set linkage to X64 Architecture (64 bits)
...
echo.  ************     Generation et lancement du deuxieme programme de test de la DLL en mode explicite.   ************
gcc -c -DNDEBUG -D_WIN32 -m64 -o testdll_explicit64.o src\testdll_explicit.c
...
REM 		-Wl,xxxxxxxx				-> Set options to linker : here, first option to generate def file, second option to generate lib file 
gcc -DBUILD_DLL -DNDEBUG -D_WIN32 -shared -o dll_core64.dll -m64 -Wl,--output-def=dll_core64.def -Wl,--out-implib,libdll_core64.dll.a src\dll_core.c 
type dll_core64.def
REM    Show list of exported symbols from a dll 
...
type dll_core64_2.def
echo.  ************     Generation et lancement du premier programme de test de la DLL en mode implicite.      *************
gcc -DNDEBUG -D_WIN32 src\testdll_implicit.c -m64 -L. -o testdll_implicit64.exe dll_core64.dll
REM 	Run test program of DLL with implicit load
testdll_implicit64.exe
echo.  ************     Generation et lancement du deuxieme programme de test de la DLL en mode explicite.     ************
gcc -DNDEBUG src\testdll_explicit.c -m64 -o testdll_explicit64.exe
REM 	Run test program of DLL with explicit load
testdll_explicit64.exe
)					
...
:FIN
echo.        Fin de la generation de la DLL et des tests avec MingW 32 bits ou 64 bits de Winlibs
